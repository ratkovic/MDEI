...
#alpha.seq <- seq(alpha.max,p,length=5)


bayesLasso(y,cbind(1,X),alpha.median,1e-8)$coef[1:10]
lm(y~X)$coef

g1<-GCV((y),cbind(1,X),alpha.seq,1e-8)
g1$beta[1:5]
g1$beta[6:12]
sparsereg(y,X,EM=T,verbose=F)$coef[1:10]
